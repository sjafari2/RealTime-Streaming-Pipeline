...
for pair in s['pairs']:
    ...
    axes[1].axhline(target,color='#777777',ls=':',lw=1)
    fig.suptitle(f"Matched pair {pair['pair']} · workload seed {pair['runs']['none']['workload_seed']}\n{int(m['config']['EXP_DURATION_SEC'])} s production · target {target:,.0f} messages/s · 2,000 SHA iterations/message",fontsize=12)
