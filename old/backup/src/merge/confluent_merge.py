...
# --- add config snapshot module path & import ---
sys.path.insert(0, "/config")
try:
    from config_snap import save_manifest
except Exception as _e:
    save_manifest = None
    print(f"[WARN] config_snap not available at /config: {_e}")
...
class HybridMerger:
    ...
    def __init__(
        ...
        self.config_hash = config_hash or "nohash"
        ...
        self.stop_at_blocks = 0  # set from CLI after constructing
    ...
    def _save_merge_metrics_row(self, throughput_MBps, late_rate, out_of_order_rate,
        ...
        if not os.path.exists(out_csv):
            ...
            try:
                ...
                missing = [c for c in df.columns if c not in existing.columns]
                if missing:
                    old = pd.read_csv(out_csv)
                    for c in missing:
                        old[c] = np.nan
                    cols_union = list(df.columns)
                    old = old.reindex(columns=cols_union)
    ...
    def _merge_csv_files(self, files):
        ...
        # Late-arrival rate: producer_ts older than previous max
        ...
        # Out-of-order & duplicates by index (best-effort if CSV path is used)
        ...
        # cfg-stamped copy
        try:
            cfg_copy = os.path.join(self.merged_dir, f"merged_{dt.strftime('%Y%m%dT%H%M%S')}_cfg{self.config_hash}.csv")
            merged_df.to_csv(cfg_copy, index=False)
        except Exception as e:
            print(f"[WARN] could not write cfg-stamped merged copy: {e}")
    ...
    def _append_block_maxima(self, rows):
        ...
        cols = ["file_name","computed_at","metric","scope","pod","block_max_value","rows_in_block","config_hash"]
        for c in cols:
        ...
        if not os.path.exists(self.block_maxima_csv):
            ...
            try:
                existing = pd.read_csv(self.block_maxima_csv, nrows=0)
                if "config_hash" not in existing.columns:
                    old = pd.read_csv(self.block_maxima_csv)
                    if "config_hash" not in old.columns:
                        old["config_hash"] = np.nan
                    old = old.reindex(columns=cols)
                    old.to_csv(self.block_maxima_csv, index=False)
            except Exception as e:
                print(f"[WARN] Schema align failed for block_maxima: {e}")
            df.to_csv(self.block_maxima_csv, mode='a', header=False, index=False)

        # per-config copy
        try:
            cfg_csv = os.path.join(self.metrics_dir, f"block_maxima_cfg{self.config_hash}.csv")
            if not os.path.exists(cfg_csv):
                df.to_csv(cfg_csv, index=False)
            else:
                df.to_csv(cfg_csv, mode='a', header=False, index=False)
        except Exception as e:
            print(f"[WARN] Could not write cfg-stamped block_maxima: {e}")
    ...
    def _compute_block_max_from_parquet(self, parquet_path: str):
        ...
        for key, g in groups:
            ...
            try:
                ...
                row = pd.DataFrame([{
                    ...
                    "metric": metric, "scope": scope, "pod": pod,
                    "n_blocks": n, "shape_c": c, "xi": -c,
                    "location_mu": loc, "scale_beta": scale,
                    "ks_D": D, "ks_pvalue": p,
                    "config_hash": self.config_hash,
                }])
                if not os.path.exists(out_csv):
                ...
                # Also write per-config copy
                try:
                    out_csv_cfg = os.path.join(self.metrics_dir, f"gev_fit_results_cfg{self.config_hash}.csv")
                    if not os.path.exists(out_csv_cfg):
                        row.to_csv(out_csv_cfg, index=False)
                    else:
                        row.to_csv(out_csv_cfg, mode='a', header=False, index=False)
                except Exception as e:
                    print(f"[WARN] Could not write cfg-stamped gev_fit_results: {e}")
                ...
                print(f"[INFO] GEV fit {metric} | {scope} | {pod or '-'} on {n} blocks: c={c:.4f}, mu={loc:.4f}, beta={scale:.4f}, KS p={p:.4f} (cfg={self.config_hash})")
            except Exception as e:
    ...
    def run(self):
        while self.running:
            try:
                ...
                # Optional stop after enough blocks
                if self.stop_at_blocks > 0:
                    try:
                        if os.path.exists(self.block_maxima_csv):
                            bdf = pd.read_csv(self.block_maxima_csv, usecols=["scope"])
                            have = int((bdf["scope"] == "global").sum())
                            if have >= int(self.stop_at_blocks):
                                print(f"[INFO] Reached {have} global block maxima ≥ stopAtBlocks={self.stop_at_blocks}. Triggering final fit and exit.")
                                self._fit_gev_if_needed()
                                self.stop()
                                break
                    except Exception as e:
                        print(f"[WARN] stopAtBlocks check failed: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--blockScope", type=str, default="global", choices=["global","per_pod"],
                        help="Scope for block maxima rows: 'global' or 'per_pod'")
    parser.add_argument("--stopAtBlocks", type=int, default=0,
                        help="If >0, stop merger after at least this many global block maxima are written")

    args = parser.parse_args()
    experiment_config = load_experiment_config()

    # --- snapshot config & stamp run ---
    CONFIG_HASH = "nohash"
    if save_manifest is not None:
        try:
            CONFIG_HASH, MANIFEST_PATH = save_manifest(args.metricsDir)
            print(f"[config] snapshot saved → {MANIFEST_PATH} (hash={CONFIG_HASH})")
            src_cfg = "/config/pipeline-configmap.yaml"
            if os.path.exists(src_cfg):
                dst_cfg = os.path.join(args.metricsDir, f"pipeline-configmap_cfg{CONFIG_HASH}.yaml")
                try:
                    shutil.copyfile(src_cfg, dst_cfg)
                    print(f"[config] copied ConfigMap → {dst_cfg}")
                except Exception as ce:
                    print(f"[WARN] could not copy ConfigMap: {ce}")
        except Exception as se:
            print(f"[WARN] save_manifest failed: {se}")
    ...
    # apply stopAtBlocks after constructing
    try:
        merger.stop_at_blocks = int(args.stopAtBlocks)
    except Exception:
        merger.stop_at_blocks = 0
