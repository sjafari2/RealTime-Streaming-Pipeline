...
## Outcome of the first bounded invocation

The 12 September 2026 invocation exhausted all six preparation attempts because their complete partition ownership differed from the frozen reference. No workload was released and no performance pair was collected. Original configuration, HPA settings and replica counts were restored; a separate runtime check found all workload applications stopped. The live rejection path was exercised, while successful admission and the pre-action check remain unvalidated live. See `experiment-records/controlled-followup-20260912/README.md` for the six maps, evidence hashes and limitations.

In the Codex session used for this invocation, initial cluster inspection timed out when the current directory was the Desktop code folder but succeeded from the project workspace. The successful invocation therefore used the final script by absolute path from `/Users/soheila/Documents/ChatGPT/Stream Processing Project`. The cause of this directory-dependent access behavior is not established. This note records the observed launch context; it does not authorize restarting the exhausted block.
