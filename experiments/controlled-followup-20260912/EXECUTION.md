...
In the execution environment used for this invocation, initial cluster inspection timed out when the current directory was the Desktop code folder but succeeded from the project workspace. The successful invocation therefore used the final script by absolute path from a separate local workspace. The cause of this directory-dependent access behavior is not established. This note records the observed launch context; it does not authorize restarting the exhausted block.
