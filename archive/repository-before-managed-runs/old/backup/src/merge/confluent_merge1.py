...
class HybridMerger:
    ...
    def __init__(
        ...
        # Where we persist block maxima (canonical path, inside run folder)
    ...
    def _save_merge_metrics_row(self, throughput_MBps, late_rate, out_of_order_rate,
        ...
        """Append one row to {run_dir}/merge_metrics.csv."""
    ...
    def _seen_keys(self):
        """Return set of (file_name, metric, scope, pod) already written to block_maxima.csv."""
    ...
    def _append_block_maxima(self, rows):
        """Append rows to the canonical {run_dir}/block_maxima.csv (no cfg duplicates)."""
        if not rows:
            return
        ...
        if not os.path.exists(self.block_maxima_csv):
            ...
            # Schema union if old file predates config_hash
            try:
    ...
    def _process_parquet_blocks(self, parquet_files: list) -> int:
        """Treat each Parquet file as one block and append maxima rows."""
    ...
    def _fit_gev_if_needed(self):
        """Periodically fit GEV per (metric, scope[, pod]) and write {run_dir}/gev_fit_results.csv."""
        now = time.time()
    ...
    def run(self):
        while self.running:
            try:
                ...
                # stop-at-blocks (global scope)
                if getattr(self, "stop_at_blocks", 0) > 0:
                    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metricsDir", required=True, help="Parent folder under which per-run folders are created")
    ...
    parser.add_argument("--blockScope", type=str, default="global", choices=["global","per_pod"],
                        help="Scope for block maxima rows: 'global' or per-pod")
    ...
    # --- snapshot config & stamp run (creates per-run folder) ---
    CONFIG_HASH = "nohash"
    RUN_DIR = args.metricsDir  # fallback
    if save_manifest is not None:
        try:
            CONFIG_HASH, MANIFEST_PATH = save_manifest(args.metricsDir)
            RUN_DIR = os.path.dirname(MANIFEST_PATH)  # <-- use the new per-run folder
            print(f"[config] snapshot saved → {MANIFEST_PATH} (hash={CONFIG_HASH})")
            # copy original configmap yaml into the RUN_DIR
            src_cfg = "/config/pipeline-configmap.yaml"
            if os.path.exists(src_cfg):
                dst_cfg = os.path.join(RUN_DIR, f"pipeline-configmap_cfg{CONFIG_HASH}.yaml")
                try:
    ...
    # Ensure RUN_DIR exists and use it for all metrics outputs
    os.makedirs(RUN_DIR, exist_ok=True)

    merger = HybridMerger(
        watch_dir=args.watchDir,
        processed_dir=args.processedDir,
        merged_dir=args.mergedDir,     # keep CSV-merge outputs separate if you use them
        metrics_dir=RUN_DIR,           # <-- all analysis files go under the per-run folder
    ...
    # set stopAtBlocks AFTER construction (fixes the previous bug)
    merger.stop_at_blocks = args.stopAtBlocks
