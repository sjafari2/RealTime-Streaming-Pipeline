...
def backlog(ax):
 ...
 ax.set(title='Processing backlog',ylabel='Uncompleted offsets');ax.set_ylim(bottom=0)
 ax.text(.99,.96,f"Lag integration coverage: {100*lag['covered_fraction']:.1f}%\nGaps denote invalid observations",transform=ax.transAxes,ha='right',va='top',fontsize=8,color='#555555')
...
with (D/'processing-backlog.csv').open('w',newline='') as stream:
 writer=csv.writer(stream);writer.writerow(['timestamp_epoch','seconds_from_evaluation_start','valid','processing_backlog_offsets'])
