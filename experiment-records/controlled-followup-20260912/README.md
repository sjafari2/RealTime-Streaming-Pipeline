...
Three earlier coordinator launch failures are preserved in audit directories block, block-authenticated and block-ready. They failed during initial read-only pod inspection before preparation or cluster mutation. In this Codex session, the same cluster command succeeded from the project workspace but timed out from the Desktop code directory; the underlying cause is not established. The completed bounded invocation used the absolute final script path with the workspace as its working directory. Authentication and browser inspection were separate initial access issues; no authentication protection was changed.
