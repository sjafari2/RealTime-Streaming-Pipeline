...
def curves(ax,name,label=None,color=None):
 for i,row in enumerate(sorted(read(name),key=lambda x:x['metric'].get('pod',''))):
  ...
  for t,v in row['values']:
   if previous is not None and t-previous>1.5*s['step']:x.append(math.nan);y.append(math.nan)
   x.append(float(t)-start);y.append(float(v));previous=t
...
def backlog(ax):
 rows=lag['snapshots'];x=[];y=[];previous=None
 for r in rows:
  if previous and r['valid'] and previous['valid'] and r.get('growth_offsets_per_second') is None:x.append(math.nan);y.append(math.nan)
  x.append(r['timestamp']-start);y.append(r.get('processing_backlog',math.nan) if r['valid'] else math.nan);previous=r
 ax.plot(x,y,color=colors[0],lw=1.5)
 ax.set(title=f"Processing backlog · {100*lag['covered_fraction']:.1f}% lag coverage\nGaps denote invalid or discontinuous observations",ylabel='Completion-frontier offsets');ax.set_ylim(bottom=0)
