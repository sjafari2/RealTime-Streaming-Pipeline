...
Frozen low-input pilot, pair A1, scheduled action scale. HPA paused; initial replicas checked. This is a preliminary action comparison, not a full adaptive controller evaluation. Interpret latency with unfinished outcomes, achieved admission, coverage, deployment/ownership events and common-horizon resource requests.

Managed status: **complete**. Full data: `results/run-20260912-095159/` in the active code folder; original event evidence remains on the Nautilus PVCs.
...
| Unique completions / evaluation second | 600.2 |
| Recorded completion mean (ms) | 172.6967 |
| Recorded completion p99 (ms) | 939.1582 |
| Provisional 99 ms cohort deadline-miss fraction | 0.6726 |
| Lag coverage fraction | 0.9167 |
| Covered mean returned-position lag (offsets) | 72.7636 |
| Peak sampled returned-position lag (offsets) | 153 |
| Recorded consumer process-seconds | 1,541.5861 |
| Resource request-time coverage fraction | 0.9331 |
