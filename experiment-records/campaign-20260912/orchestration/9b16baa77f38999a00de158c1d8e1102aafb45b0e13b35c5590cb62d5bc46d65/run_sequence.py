...
protocol = json.loads((E/'comparison-protocol.json').read_text())
assert protocol['calibration_qualified'] is True
assert len(protocol['trials']) <= 8
state_path = A/'sequence-status.json'
assert not state_path.exists(), 'Inspect any existing sequence before resuming it'
state = dict(started_epoch=time.time(), status='running', protocol=str(E/'comparison-protocol.json'), trials=[])
...
try:
    for trial in protocol['trials']:
        ...
        guard_command=[sys.executable,'-u','review/preliminary-campaign-20260912/guard_trial.py',
                       '--label',label,'--monitoring-transition-seconds','60']
        ...
        if guard_failed or summary['validity_failures'] or fraction<.98 or lag['covered_fraction']<.9:
