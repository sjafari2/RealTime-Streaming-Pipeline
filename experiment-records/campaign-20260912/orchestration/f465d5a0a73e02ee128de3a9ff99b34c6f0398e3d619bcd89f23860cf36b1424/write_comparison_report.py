...
for pair in s['pairs']:
 ...
 for action,row in [('Keep 3',n),('Scale',c)]:
  diagnostic='Backlog-threshold diagnostic' if index.get('family')=='low-input' else 'Recovery'
  lines += [f"{action} quality flags: {('; '.join(row['quality_flags'])) or 'none at the predeclared coverage screens'}. {diagnostic}: `{json.dumps(row.get('recovery'),sort_keys=True)}`."]
 if index.get('family')=='low-input':
  lines += ['', '| Condition | Valid pre-action backlog range (offsets) | Valid full-evaluation backlog range (offsets) |', '|---|---:|---:|']
  for action,row in [('Keep 3',n),('Scale',c)]:
   directory=Path(row['directory']);lag=json.loads((directory/'lag-summary.json').read_text());anchor=row['intervention']['at_epoch']
   observed=[z['processing_backlog'] for z in lag['snapshots'] if z['valid']]
   before=[z['processing_backlog'] for z in lag['snapshots'] if z['valid'] and z['timestamp']<=anchor]
   interval=lambda values:f'{min(values):,.0f}–{max(values):,.0f}' if values else 'Unavailable'
   lines.append(f"| {action} | {interval(before)} | {interval(observed)} |")
  lines += ['', 'The saved analyzer status `recovered` denotes the defined backlog-threshold hold. When pre-action backlog was already below the threshold, interpret its confirmation as a health check, not recovery from observed prior overload. Ranges above describe valid samples only; they do not fill missing transition intervals or establish a universal capacity margin.', '']
