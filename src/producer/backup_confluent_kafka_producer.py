        ...
        while self.running:
            try:
                ...
                if elapsed > 0:
                    ...
                    print(f"[METRIC] Elapsed: {elapsed:.2f}s | Sent: {self.msg_sent} msgs | Rate: {msg_rate:.2f} msg/s | Real Throughput: {mb_rate:.4f} MB/s")
    ...
    def send_message(self, topic, message, headers=None):
        ...
        while not delivered and self.running:
            try:
                ...
                delivered = True
                self.msg_sent += 1
                self.bytes_sent += len(message_bytes)  # using real payload size for throughput calc
