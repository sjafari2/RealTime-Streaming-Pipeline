# Final BSP-aligned `HybridMerger` with clear informative prints for debugging and monitoring
...
class HybridMerger:
    def __init__(self, watch_dir, processed_dir, merged_dir, metrics_dir, min_files, interval_sec):
        ...
        print(f"[MERGE] HybridMerger started.")
        print(f"[MERGE] Watching directory: {self.watch_dir}")

    def get_unprocessed_files(self):
        files = sorted([f for f in os.listdir(self.watch_dir) if f.endswith(".csv") and not f.startswith(".tmp_")])
        if files:
    ...
    def move_to_processed(self, filepath):
        ...
        print(f"[MERGE] Moved {os.path.basename(filepath)} to processed directory.")
    ...
    def merge_csv_files(self, files):
        print(f"[MERGE] Starting merge on {len(files)} files")
        dfs = []
        total_bytes = 0
        ...
        if not dfs:
            print(f"[MERGE] No valid files found for merging.")
            return None, None
        ...
        print(f"[MERGE] Saved merged file: {merged_filename} with {len(merged_df)} rows.")
        print(f"[MERGE] Throughput: {throughput_MBps:.2f} MB/s, Late rate: {late_arrival_rate:.3f}, Out-of-order: {out_of_order_rate:.3f}, Duplicate: {duplicate_rate:.3f}")
        ...
        print(f"[MERGE] Batch metrics appended to {metrics_file}")
        ...
        while True:
            try:
                ...
                if unprocessed_files and (len(unprocessed_files) >= self.min_files or now - self.last_merge_time >= self.interval):
                    print(f"[MERGE] Ready to process batch (files: {len(unprocessed_files)}, elapsed: {now - self.last_merge_time:.1f}s).")
                    ...
                    if merged_path:
                        ...
                        print("[MERGE] Skipping move to processed, no valid merge.")
                else:
                    print(f"[MERGE] Waiting for next batch... ({len(unprocessed_files)} files detected)")
                    time.sleep(2)
            except KeyboardInterrupt:
                print("[MERGE] Keyboard interrupt received, stopping merger.")
                break
            except Exception as e:
                print(f"[ERROR] Exception in merger loop: {e}")
                time.sleep(2)
