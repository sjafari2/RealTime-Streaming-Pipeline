...
# Export .data/* as env vars prefixed with data_
eval $(yq eval '.data | to_entries | map("export data_" + .key + "=" + (.value | @sh)) | .[]' "$CONFIG_FILE")
...
python3 confluent_kafka_producer.py \
  ...
  --pacingMode "${data_PACING_MODE}" \
  --pacingTickSec "${data_PACING_TICK_SEC}" \
  --minBurst "${data_MIN_BURST}" \
  --maxBurst "${data_MAX_BURST}" \
